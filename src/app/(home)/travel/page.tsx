...
const CityItem = ({ city, onMouseEnter }: CityItemProps) => {
  ...
  return (
    <div
      key={city.id}
      className="w-full p-5 bg-muted hover:bg-muted-hover rounded-xl flex justify-between items-center cursor-pointer group transition-all duration-150 ease-[cubic-bezier(0.22, 1, 0.36, 1)] flex-1 overflow-hidden"
      onMouseEnter={() => onMouseEnter(city)}
      onClick={() => router.push(`/travel/${city.city}`)}
    >
      <p className="text-xs lg:text-sm">{city.city}</p>
      ...
      <div className="relative overflow-hidden min-w-[300px] flex justify-end">
        <div className="flex items-center gap-2 transform transition-transform duration-200 ease-in-out group-hover:-translate-x-7">
          <span className="font-light text-xs lg:text-sm whitespace-nowrap text-right">
            <TextScroll className="w-24 lg:w-full">{city.country}</TextScroll>
          </span>
        </div>
